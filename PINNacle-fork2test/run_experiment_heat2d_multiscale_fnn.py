# run_experiment_heat2d_multiscale_fnn.py
# Standard FNN experiment for 2D Multiscale Heat equation
...
def get_model():
    ...
    net = FNN(
        layer_sizes=[3, 256, 256, 256, 256, 1],  # Deeper network for multiscale problem
...
if __name__ == "__main__":
    ...
    # Initialize trainer
    trainer = Trainer(exp_name="Heat2D_Multiscale_FNN", device="0")
    
    # Add experiment task
    trainer.add_task(get_model, train_args)

    print(">>> 开始实验！2D多尺度热方程 + 标准前馈神经网络")
    trainer.train_all()
    print(">>> 实验完成！")
    ...
    exp_name = "Heat2D_Multiscale_FNN"
