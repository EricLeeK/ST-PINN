# run_experiment_wave2d_longtime_fnn.py
# Standard FNN experiment for 2D Wave equation long-time dynamics
...
def get_model():
    ...
    net = FNN(
        layer_sizes=[3, 256, 256, 256, 256, 1],  # Deep network for long-time dynamics
...
if __name__ == "__main__":
    ...
    # Initialize trainer
    trainer = Trainer(exp_name="Wave2D_Longtime_FNN", device="0")
    
    # Add experiment task
    trainer.add_task(get_model, train_args)

    print(">>> 开始实验！2D长时间波动方程 + 标准前馈神经网络")
    trainer.train_all()
    print(">>> 实验完成！")
