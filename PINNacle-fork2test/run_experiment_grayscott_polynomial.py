# run_experiment_grayscott_polynomial.py
# Experiment for Gray-Scott reaction-diffusion system using Polynomial time basis
...
def get_model():
    ...
    # Note: GrayScott has input_dim=3 (x, y, t) and output_dim=2 (u, v)
    net = SeparatedNetPolynomial(
        layer_sizes=[pde.input_dim, 0, pde.output_dim],
        activation=None, 
        kernel_initializer=None,
        spatial_layers=[256, 256, 256, 256],  # Very deep network for complex patterns
        poly_degree=50                        # Very high degree for long-time coverage
    )
...
if __name__ == "__main__":
    ...
    # Initialize trainer
    trainer = Trainer(exp_name="GrayScott_Polynomial", device="0")
    
    # Add experiment task
    trainer.add_task(get_model, train_args)

    print(">>> 开始实验！Gray-Scott反应扩散系统 + 多项式时间基")
    trainer.train_all()
    print(">>> 实验完成！")
