# run_experiment_heat2d_fnn.py
# Standard FNN experiment for 2D Heat equation with varying coefficients
...
def get_model():
    ...
    # Input: [x, y, t] (3D), Output: [u] (1D)
    net = FNN(
        layer_sizes=[3, 128, 128, 128, 1],  # Standard architecture: 3 -> 128x3 -> 1
...
if __name__ == "__main__":
    ...
    # Initialize trainer
    trainer = Trainer(exp_name="Heat2D_FNN", device="0")
    
    # Add experiment task
    trainer.add_task(get_model, train_args)

    print(">>> 开始实验！2D变系数热方程 + 标准前馈神经网络")
    trainer.train_all()
    print(">>> 实验完成！")
